...
2. **Viator 区块**：从 `/ferry/` 移除「当地体验 · 一日游」；`/trekking/` 在**路线卡片之后**挂载 `AffiliateUI.trekkingSectionHtml`（Klook 徒步 + Viator 日归），白底 `panel-aux` 可折叠。
3. **联盟数据**：`viator_jetfoil_day.pages` 去掉 `ferry`，保留 `trekking` / `without-car`。
4. **船运页底部**：仅保留 Klook「优惠购票」折叠 +「官方链接」；Viator 不再插入。

## 2026-06-27

- 分销区从「季节与路线之间」挪到**徒步路线卡片下方**（与官方资料同级白卡片），避免被误认为消失。
