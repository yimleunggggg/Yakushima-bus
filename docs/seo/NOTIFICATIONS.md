...
前置：已完成 [GOOGLE_SETUP.md](GOOGLE_SETUP.md) 且 workflow 能跑通。
...
| 两轮均失败 | ntfy **高优先级**：`[YakuBus] 日报失败 · 请人工处理` + Actions 链接 |

第 2 轮成功时 workflow 显示 **绿**，标题带「重试成功」。**与你电脑开不开机无关**。

### 站点挂了谁告警（2026-05）

| 渠道 | 何时 |
|------|------|
| **ntfy urgent** | 每日 Actions 开头 HTTP 探测 `/` 等四页非 200 → **立即推送**（不等日报） |
| **日报 §一** | 显示「线上站点异常」+ P0 |
| **GSC** | 你手动「测试实际网址」才会看到（**不会主动推送**） |

本地自检：`python3 scripts/seo_site_uptime.py`
