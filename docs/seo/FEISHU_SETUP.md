...
2. **权限**：`sheets:spreadsheet`、`drive:drive`；（可选追踪 doc）`docx:document`
...
1. 建文件夹 **YakuBus SEO**
2. URL 里 `folder/` 后即为 `FEISHU_FOLDER_TOKEN`（例：`E2LWfHU8El45fydPcvTcDaa9nSg`）
...
| `FEISHU_FOLDER_TOKEN` | 首次 init | 文件夹 token |
| `FEISHU_SHEET_TOKEN` | **是** | 固定表格 token；日报/周报都只更新这一张 |
...
python3 scripts/seo_feishu_weekly_sheet.py init   # 或 daily init，同一张表
```

或飞书手动建表 **YakuBus SEO 数据** → URL 里 `shtcn...` 即为 token。

### 4.2 绑定 token

1. GitHub Secret **`FEISHU_SHEET_TOKEN`** = 表格 token
2. commit `docs/seo/feishu-sheet.json`（init 自动生成）
